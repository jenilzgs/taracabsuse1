...
import 'package:ride_sharing_user_app/view/screens/location/model/place_details_model.dart';
...
class LocationController extends GetxController implements GetxService {
        ...
        _locationSubscription = Geolocator.getPositionStream().listen((newLocalData) {
          if (mapController != null) {
            mapController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(
                ...
                tilt: 0,
                zoom: 16)));
            Get.find<MapController>().updateMarkerAndCircle(newLocalData);
  ...
  Future <void> saveUserAddress(Address? address) async {
    locationRepo.saveUserAddress(address);
  }

  Address? getUserAddress() {
    Address? address;
    if(locationRepo.getUserAddress() != null) {
      address = Address.fromJson(jsonDecode(locationRepo.getUserAddress()!));
    }
    return address;
  }
  ...
  void updatePosition(CameraPosition position, bool fromAddressScreen, LocationType? type) async {
  ...
  Future<bool> checkPermission(Function onTap) async {
    ...
    if(permission == LocationPermission.denied) {
      ...
      Get.dialog(ConfirmationDialog(description: 'you_denied_location_permission'.tr, onYesPressed: () async {
        Get.back();
