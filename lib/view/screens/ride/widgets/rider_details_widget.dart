...
import 'package:ride_sharing_user_app/view/screens/ride/widgets/bidding_list_screen.dart';
...
class _RiderDetailsWidgetState extends State<RiderDetailsWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(onTap: ()=> Get.off(()=> BiddingListScreen(tripId: widget.tripId, fromList: true)),
        ...
        builder: (rideController) {
          return Padding(padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeDefault),
            child: Container(padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
              decoration: BoxDecoration(color: Get.isDarkMode? Theme.of(context).canvasColor :Theme.of(context).cardColor,
              ...
              child: Column(children: [
                        ...
                        Text.rich(TextSpan(
                            style: textRegular.copyWith(fontSize: Dimensions.fontSizeLarge,
                                color: Theme.of(context).textTheme.bodyMedium!.color!.withOpacity(0.8)), children:  [
                                  ...
                                  color: Theme.of(context).primaryColorDark,fontSize: Dimensions.fontSizeDefault)),
                              const TextSpan(text: " "),

                              WidgetSpan(child: Icon(Icons.star,color: Theme.of(context).colorScheme.primaryContainer,size: 15),
                                  alignment: PlaceholderAlignment.middle),
                              TextSpan(text:widget.bidding.driverAvgRating != null? double.parse(widget.bidding.driverAvgRating.toString()).toStringAsFixed(1) : '0',
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        ...
                        if(rideController.remainingDistanceModel != null && rideController.remainingDistanceModel!.isNotEmpty)
                          Text("${rideController.remainingDistanceModel![0].distanceText}",
                              style: textMedium.copyWith(fontSize: Dimensions.fontSizeDefault,
                                  color: Theme.of(context).primaryColorDark.withOpacity(0.6)),
                              overflow: TextOverflow.ellipsis),
                      ],
                ...
                Row(children: [
                    Expanded(child: CustomButton(
                      ...
                      boldText: false,
                      onPressed: (){
                        rideController.ignoreBidding(widget.bidding.id!).then((value){
                          if(value.statusCode == 200){
                            rideController.getBiddingList(widget.tripId, 1);
                          }
                        });
                      },
                    )),
